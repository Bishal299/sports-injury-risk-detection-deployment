...
from app.schemas.analysis_result import AnalysisResultResponse, AnalysisStatusResponse
...
def trigger_analysis(
    ...
    if not existing:
        ...
        existing.progress = 5
        existing.stage = "Initializing video pipeline..."
    ...
    background_tasks.add_task(run_movement_analysis_pipeline, video_id=video_id)
...
def download_analysis_csv(
    ...
    return FileResponse(
        path=csv_path,
        media_type="text/csv",
        filename=f"movement_analysis_{video_id}.csv"
    )
