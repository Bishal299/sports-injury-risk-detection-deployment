...
def run_e2e_tests():
    ...
    if not videos:
        ...
        existing_vids = [
            os.path.join("uploads", "videos", f)
            for f in os.listdir(os.path.join("uploads", "videos"))
            if f.endswith(".mp4")
        ]
        sample_path = existing_vids[0] if existing_vids else "uploads/videos/5b9e8da3-bc62-4774-b9da-6a613036e181.mp4"
