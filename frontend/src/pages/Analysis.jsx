...
function Analysis() {
  ...
  // Sync split video players
  const handleSplitPlay = () => {
    if (splitOrigRef.current && splitSkelRef.current) {
      splitOrigRef.current.play();
      splitSkelRef.current.play();
    }
  };

  const handleSplitPause = () => {
    if (splitOrigRef.current && splitSkelRef.current) {
      splitOrigRef.current.pause();
      splitSkelRef.current.pause();
    }
  };

  const handleSplitSeek = (e) => {
    const time = e.target.currentTime;
    if (splitOrigRef.current) splitOrigRef.current.currentTime = time;
    if (splitSkelRef.current) splitSkelRef.current.currentTime = time;
  };
  ...
  const force = summary.force_estimation || {};
  const alignment = summary.alignment || {};
  const recommendations = analysis?.recommendations || [];

  const overallScore = analysis?.movement_quality || 80.0;
  const riskLevel = analysis?.risk_level || "Moderate Risk";
        ...
        <div className="analysis-header-row">
          <div className="analysis-title-group">
            ...
            <div className="analysis-meta-chips">
              ...
              <div className="meta-chip">
                <strong>Activity:</strong> {video?.activity || "Movement Test"}
              </div>
              {video?.duration && (
                <div className="meta-chip">
                  <strong>Duration:</strong> {Number(video.duration).toFixed(1)}s ({video.fps || 30} fps)
                </div>
              )}
              <div className={`status-badge ${status}`}>
                <span className="pulse-dot"></span>
                {status === "completed" ? "Analysis Complete" : (status === "processing" ? "Processing Video" : status)}
              </div>
            </div>
          </div>

          <div className="analysis-header-actions">
            <button className="secondary-button" onClick={() => navigate("/my-videos")}>
              <ArrowLeft size={16} />
        ...
        {status === "completed" && analysis && (
          <>
            ...
            <div className="analysis-top-grid">
              ...
              {/* Extracted Biomechanical Features Summary Card */}
              <div className="score-card-container">
                <div>
                  <div className="score-main-row" style={{ alignItems: "flex-start", gap: "16px" }}>
                    ...
                    <div className="score-info-box">
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                        <span className="table-status-badge optimal">
                          {video?.fps || 30} FPS Kinematics
                        </span>
                        <span className="table-status-badge mild">
                          33 Pose Landmarks
                        </span>
                      </div>
                      <h3 style={{ fontSize: "1.2rem", margin: "4px 0 6px" }}>Extracted Movement Features</h3>
                      <p className="score-summary-text">
                        Automated kinematic extraction across high-speed video frames including joint angles, dynamic knee valgus, pelvic tilt stability, and bilateral symmetry.
                      </p>
                    </div>
                  </div>

                  {/* Extracted Features Quick Grid */}
                  <div className="quick-metrics-grid" style={{ marginTop: "16px" }}>
                    <div className="mini-metric-box">
                      <span className="mini-metric-label">Max Knee Valgus</span>
                      <span className="mini-metric-val">{valgus.max_deviation?.toFixed(1) || 0}° <small style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>dev</small></span>
                    </div>
                    <div className="mini-metric-box">
                      <span className="mini-metric-label">Avg Pelvic Tilt</span>
                      <span className="mini-metric-val">{hip.avg_pelvic_tilt_deg?.toFixed(1) || 0}° <small style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>tilt</small></span>
                    </div>
                    <div className="mini-metric-box">
                      <span className="mini-metric-label">Avg Trunk Lean</span>
                      <span className="mini-metric-val">{trunk.avg_trunk_lean_deg?.toFixed(1) || 0}° <small style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>lean</small></span>
                    </div>
                    <div className="mini-metric-box">
                      <span className="mini-metric-label">Symmetry Index</span>
                      <span className="mini-metric-val">{symmetry.overall_symmetry_score?.toFixed(0) || 90}%</span>
                    </div>
                    <div className="mini-metric-box">
                      <span className="mini-metric-label">Normalized Stride</span>
                      <span className="mini-metric-val">{stride.normalized_stride_length?.toFixed(2) || "0.50"}</span>
                    </div>
                    <div className="mini-metric-box">
                      <span className="mini-metric-label">Peak Force Proxy</span>
                      <span className="mini-metric-val">{force.peak_force_n || 0} N</span>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ marginTop: "16px", paddingTop: "14px", borderTop: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>
                    Analysis ID: {String(analysis.analysis_id).slice(0, 8)}...
                  </span>
                  <button className="secondary-button" style={{ padding: "6px 12px", fontSize: "0.8rem" }} onClick={() => setActiveTab("reports")}>
                    <Download size={14} />
                    <span>View CSV & Reports</span>
                  </button>
                </div>
              </div>
            </div>
                ...
                <div className="card-panel">
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", marginTop: "18px" }}>
                    ...
                    <div className="mini-metric-box">
                      ...
                      <strong className="mini-metric-val">{force.estimated_body_mass_kg || 70} kg</strong>
                    </div>
                  </div>
              ...
              <div className="tab-panel">
                <div className="reports-action-row">
                  ...
                  <div className="report-download-card">
                    ...
                    >
                      <Download size={16} />
                      {downloadingFile === 'csv' ? "Downloading CSV..." : "Download CSV Data"}
                    </button>
                  </div>
                ...
                <div className="card-panel">
                  <div className="card-panel-header">
                    <h3>CSV Dataset Preview (First 15 Recorded Frames)</h3>
                  </div>
